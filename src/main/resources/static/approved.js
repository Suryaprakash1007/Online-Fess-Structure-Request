document.addEventListener("DOMContentLoaded", async () => {
    ...
    async function loadApproved() {
      tableBody.innerHTML = "";
      try {
        const response = await fetch("http://localhost:8081/api/students/approved");
        ...
        students.forEach(stu => {
          ...
          row.innerHTML = `
            ...
            <td>${stu.status}</td>
            <td>
              <a class="file-link" href="http://localhost:8081/api/students/${stu.id}/file" target="_blank">View</a>
            </td>
          `;
